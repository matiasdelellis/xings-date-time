 ...
 **/
static void
xdt_debug_ignore_cb (const gchar *log_domain, GLogLevelFlags log_level,
...
/**
 * xdt_debug_handler_cb:
 **/
static void
xdt_debug_handler_cb (const gchar *log_domain, GLogLevelFlags log_level,
		     const gchar *message, gpointer user_data)
{
	gchar str_time[255];
	time_t the_time;
 ...
 */
void
xdt_debug_add_log_domain (const gchar *log_domain)
{
	if (_verbose) {
		...
		g_log_set_handler (log_domain,
				   ...
				   G_LOG_LEVEL_DEBUG |
				   G_LOG_LEVEL_WARNING,
				   xdt_debug_handler_cb, NULL);
	} else {
		/* hide all debugging */
		g_log_set_handler (log_domain,
				   G_LOG_LEVEL_DEBUG | G_LOG_LEVEL_WARNING,
				   xdt_debug_ignore_cb, NULL);
	}
}
